...
5. 所有网站安装器必须提供可编辑的数据库表前缀；全站统一解析表名，禁止硬编码完整表名，旧站升级沿用原前缀。
6. 所有 Android APK 主要交付物必须已经签名、可直接安装；已有应用沿用原签名，新项目签名文件私密交付给用户。
7. 用户说“加固 APK”时，直接完成当前环境和项目兼容范围内能做到的最好加固，并使用同一份交付签名完成最终签名和升级验证。
8. 修 Bug 前必须先复现、读取完整错误与日志、追踪根因；每轮只验证一个假设，不得堆叠猜测性补丁。
9. 同一问题连续三次修复失败后必须停止盲修，提交阶段报告并复查架构、状态、依赖和环境。
10. 没有当前版本的新鲜验证证据，不得宣称“已修复”；必须验证原始复现、完整构建、安装/部署和关联回归。
